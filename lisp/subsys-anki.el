;;; subsys-anki.el --- Anki subsystem  -*- lexical-binding: t -*-
;;; Commentary:
;;
;; Configure the Anki subsystem.
;;
;; In `org-mode', we make available Anki (apps.ankiweb.net) as a tool for spaced
;; repetition.
;;
;; The `anki-editor' minor mode enables editing of Anki notes in Org mode. Sync
;; is unidirectional, i.e., the notes can only be pushed from Org, but not the
;; other way.
;;
;; To use the `anki-editor' mode, the AnkiConnect add-on needs to be installed
;; on the Anki app client (Tools -> Add-ons).
;;
;; To style cloze deletions, enable `anki-editor-cloze-mode' within an Org
;; document:
;;
;;   # Local Variables:
;;   # eval: (anki-editor-cloze-mode 1)
;;   # End:
;;
;;; Code:
